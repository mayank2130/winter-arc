  ...
  const checkoutProduct = async (
    ...
    if (useDynamicPaymentLinks) {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_URL}/api/checkout/${productType}?productId=${productId}`,
        {
          cache: "no-store",
        }
      );
      ...
      let checkoutUrl = `https://test.checkout.dodopayments.com/buy/${productId}?quantity=1&redirect_url=${process.env.NEXT_PUBLIC_URL}`;
